...
import { filter, pipe, skip, switchMap } from 'rxjs';
...
@Component({
  ...
  standalone: true,
  imports: [PageHeaderComponent, SharedMaterialModule, SharedPipesModule, TagComponent],
})
export class UserListComponent implements OnInit {
  ...
  public openCreateUserDialog() {
    ...
    dialogRef
      ...
      .pipe(
        filter(res => !!res),
        switchMap(res => this.userService.createUser(res.username, res.password, res.roles))
      )
      .subscribe(res => {
        if (res.status === 200) {
